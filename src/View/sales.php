                                        ...
                                        <fieldset>
                                            ...
                                            <div id="cat-list" class="py-1">
                                                ...
                                                <button class="btn btn-default btn-xs rounded-pill px-2 cat_btn mx-3 col-lg-3 col-md-4 col-sm-6 col-xs-10
                                                 <?= isset($category) ? "bg-gradient-warning text-light" : "bg-gradient-light border" ?>" type="button" data-id='<?= $category->id ?>'><?= $category->name ?></button>
                                                <?php } 
                                                ?>
                                            </div>
                                        </fieldset>
                                        <fieldset>
                                            ...
                                            <div id="item-list" class="py-1 overflow-auto">
                                                <div class="row row-cols-xl-3 row-cols-md-2 row-cols-sm-1 gy-2 gx-2">
                                                    <?php
                                                        foreach($data['allMenues'] as $menu){
                                                    ?>
                                                    <div class="col <?= isset($menu) && $menu == $menu->category_id ? "" : "d-none" ?> menu-item" data-cat-id='<?= $menu->category_id ?>'>
                                                        <button class="btn btn-default btn-block btn-xs rounded-pill px-2 bg-gradient-light border my item-btn" type="button" data-id='<?= $menu->id ?>' data-price='<?= $menu->price ?>'>
        ...
        $(function() {
            ...
            $('#sales-form').submit(function(e) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    error: err => {
                        console.log(_base_url_ + "sales/makeOrder")
                    ...
                    success: function(resp) {
                        if (resp.status == 'success') {
                            ...
                            setTimeout(() => {
                                ...
                                var nw = window.open(_base_url_ + `sales/receipt/${resp.order_id}`, '_blank', "width=" + ($(window).width() * .8) + ",left=" + ($(window).width() * .1) + ",height=" + ($(window).height() * .8) + ",top=" + ($(window).height() * .1))
                                setTimeout(() => {
                                    nw.print()
                                    setTimeout(() => {
                                        nw.close()
